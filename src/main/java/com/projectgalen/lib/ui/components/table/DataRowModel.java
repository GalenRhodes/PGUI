...
import com.projectgalen.lib.ui.components.table.misc.DataModel;
...
public interface DataRowModel<T> {
    ...
    @NotNull DataModel<T> getDataModel();
    ...
    void setColumnAttributes(@NotNull DefaultTableCellRenderer cellRenderer, @NotNull Component rendererComponent, @NotNull JTable table, @NotNull T object, int columnIndex, boolean isSelected);

    void setColumnValue(@NotNull T obj, int rowIndex, int columnIndex, @Nullable Object newValue);

    void setModelProvider(@NotNull Supplier<DataModel<T>> modelProvider);
}
